...
def _render_page_markdown(meta: dict, body_md: str) -> str:
    """Return a clean Markdown rendering of a page, suitable for LLM consumption.

    Strips the YAML frontmatter and prepends an `# H1` plus a one-line
    italicised subtitle, so the file reads as a self-contained document.
    """
    parts: list[str] = [f'# {meta["h1"]}']
    subtitle = meta.get('subtitle')
    if subtitle:
        parts.append(f'_{subtitle}_')
    parts.append(body_md.rstrip())
    return '\n\n'.join(parts) + '\n'


def _build_llms_txt(site: dict, sources_by_slug: dict[str, dict]) -> str:
    """Build the `llms.txt` index per https://llmstxt.org.

    Sections mirror the Diátaxis groups in `_site.yaml`. Each link points at
    the per-page `.md` source (also generated by this build), so an agent
    can fetch raw Markdown rather than scraping HTML.
    """
    site_url = site['site_url'].rstrip('/') + '/'
    index_meta = sources_by_slug['index']['meta']
    lines: list[str] = [
        f'# {index_meta["h1"]}',
        '',
        f'> {index_meta["subtitle"]}',
        '',
        (
            'This file is an [llms.txt](https://llmstxt.org) index of the '
            'borescope documentation. Each link points at the Markdown '
            'source of a page on the deployed docs site.'
        ),
        '',
    ]
    for section_key, cfg in site['sections'].items():
        ...
        for page in cfg.get('pages', []):
            slug = page['slug']
            entry = sources_by_slug.get(slug)
            if entry is None:
                continue
            url = f'{site_url}docs/{slug}.md'
            description = entry['meta'].get('subtitle') or entry['meta'].get('description', '')
            lines.append(f'- [{page["label"]}]({url}): {description}')
        lines.append('')
    return '\n'.join(lines).rstrip() + '\n'
...
def _build_all(
    ...
    """Build every page. Returns (html_outputs, auxiliary_outputs).

    `out_dir` receives per-page HTML and the per-page `.md` companion files.
    `site_root` (defaults to `out_dir.parent`) receives site-level files
    like `llms.txt` so they sit at the public site root.
    """
    ...
    html_built: list[tuple[pathlib.Path, str]] = []
    aux_built: list[tuple[pathlib.Path, str]] = []
    sources_by_slug: dict[str, dict] = {}
    for src in _iter_sources():
        raw = src.read_text(encoding='utf-8')
        meta, body_md = _split_frontmatter(raw)
        sources_by_slug[src.stem] = {'meta': meta, 'body': body_md}

        rendered = _render_page(src, site, env, md, all_pages)
        html_built.append((out_dir / f'{src.stem}.html', rendered))

        md_text = _render_page_markdown(meta, body_md)
        aux_built.append((out_dir / f'{src.stem}.md', md_text))

    llms_txt = _build_llms_txt(site, sources_by_slug)
    aux_built.append((site_root / 'llms.txt', llms_txt))
    return html_built, aux_built
...
def _check(out_dir: pathlib.Path, semantic: bool) -> int:
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        html_built, aux_built = _build_all(tmp_docs, site_root=tmp_path)
        for dest, rebuilt in html_built:
            rel = dest.relative_to(tmp_docs)
            committed = out_dir / rel
            if not committed.exists():
                print(f'[new]       docs/{rel}')
                mismatches.append(rel)
                continue
            existing = committed.read_text(encoding='utf-8')
            if semantic:
                if _flatten_dom(existing) != _flatten_dom(rebuilt):
                    mismatches.append(rel)
                    print(f'[differs]   docs/{rel}  (semantic DOM mismatch)')
            else:
                if existing != rebuilt:
                    mismatches.append(rel)
                    diff = difflib.unified_diff(
                        ...
                        fromfile=f'committed/docs/{rel}',
                        tofile=f'rebuilt/docs/{rel}',
                        n=2,
                    )
                    sys.stdout.write(''.join(diff))
                    print(f'[differs]   docs/{rel}')
        # Auxiliary outputs (per-page .md, llms.txt) are always compared
        # literally — they have no DOM and any drift is meaningful.
        for dest, rebuilt in aux_built:
...
def main() -> int:
    ...
    html_built, aux_built = _build_all(OUT_DIR)
    for dest, rendered in html_built + aux_built:
