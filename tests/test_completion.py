...
from cascade.shell.completion import CascadeCompleter


def _completions(completer: CascadeCompleter, text: str) -> list[tuple[str, int]]:
...
def test_complete_command_names_empty(ctx):
    completer = CascadeCompleter(["ls", "cat", "services"], ctx)
...
def test_complete_command_names_prefix(ctx):
    completer = CascadeCompleter(["ls", "cat", "services", "start"], ctx)
...
def test_complete_path_top_level(transport, ctx):
    ...
    completer = CascadeCompleter(["ls"], ctx)
...
def test_complete_path_with_prefix(transport, ctx):
    ...
    completer = CascadeCompleter(["cat"], ctx)
